...
export const removeEmployee = async (req: Request, res: Response) => {
  try {
    ...
    if (user.role != 'Admin') {
        res.status(401).json({ message : "Attenzione: operazione non autorizzabile"})
      return;
    }
    ...
    if (employeeToBeDeleted.Role === 'Admin'){
      res.status(401).json({ message : "Attenzione: Dipendente Admin non cancellabile"});
      return;
    }
    ...
    res.status(200).json({ message : "Successo, Dipendente rimosso", employee : employeeToBeDeleted.First_Name + " " + employeeToBeDeleted.Last_Name
    })
